...
// to zero velocity.
TeknicMotor::TeknicMotor(SimpleNode &node, ros::Publisher telem, string name) : _node(node) {
  _name = name;
...
vector<NavMotor> init_motors(string path, NavMotor *&augerMotor,
                             ...
                             NavMotor *&dumperLMotor, NavMotor *&dumperRMotor, ros::Publisher telemetry_pub) {
  ...
  motors.push_back(
                   NavMotor(new TeknicMotor(nodes.at(MotorIdent::LocomotionL), telemetry_pub, "loco_left"), "loco_left", -1, -1));
  motors.push_back(
                   NavMotor(new TeknicMotor(nodes.at(MotorIdent::LocomotionR), telemetry_pub, "loco_right"), "loco_right", 1, 1));

  augerMotor = new NavMotor(new TeknicMotor(nodes.at(MotorIdent::Auger), telemetry_pub, "auger_rotation"), "auger_rotation", 0, 1);
  depthLMotor =
    new NavMotor(new TeknicMotor(nodes.at(MotorIdent::DepthL), telemetry_pub, "L_depth"), "L_depth", 0, 1);
  depthRMotor =
    new NavMotor(new TeknicMotor(nodes.at(MotorIdent::DepthR), telemetry_pub, "R_depth"),"R_depth" , 0, 1);
  dumperLMotor =
    new NavMotor(new TeknicMotor(nodes.at(MotorIdent::DumpL), telemetry_pub, "left_dump"), "left_dump" ,0, 1);
  dumperRMotor = new NavMotor(
      new TeknicMotor(nodes.at(MotorIdent::DumpR), telemetry_pub, "right_dump"),"right_dump", 0, 1);
...
void TeknicMotor::motor_manager() {
  while (true) {
    ...
    if ( _name == "loco_left" ){
      ...
      msg.name = {_name};
      msg.position = {_node.position * LOCO_CONVERSION_FACTOR / (double) 10}; // encoder count
      msg.velocity = {_node.velocity * LOCO_CONVERSION_FACTOR_VEL / (double) 10}; // velocity
      ...
      sensor_msgs::JointState msg;
      msg.name = {_name};
      msg.position = {_node.position * LOCO_CONVERSION_FACTOR / (double) 50}; // encoder count
      msg.velocity = {_node.velocity * LOCO_CONVERSION_FACTOR_VEL / (double) 50}; // velocity
      msg.effort = {_node.rms};
      _telem.publish(msg);
      ...
      sensor_msgs::JointState msg;
      msg.name = {_name};
      msg.position = {_node.position * DEPTH_CONVERSION_FACTOR}; // encoder count
